...
# filtered.data.200 <- win.data_200[keep_large,]
# 
# rowData(filtered.data) <-  as.data.frame(res)
# # rowData(filtered.data) <- cbind(rowData(filtered.data), res)
# rowData(filtered.data.200) <- as.data.frame(res.win.large)
#
##------- instead of these we can run single command after these steps:
# 
# merged <-
#   mergeWindows(
#     filtered.data,
...
# merged.max <- mergeWindows(filtered.data, tol=1000L, max.width=3000L)
# summary(width(merged.max$regions))
# 
# ## Combining window-level p-values
# tabcom <- combineTests(merged$ids, res$table)
# is.sig.region <- tabcom$FDR <= 0.05 
# summary(is.sig.region)
# table(tabcom$direction[is.sig.region])  ## down
# 
# # This reports the index of the window with the lowest p-value in each cluster as well as the associated statistics.
# tab.best <- getBestTest(merged$ids, res$table) 
# head(tab.best)
# The corrected p-value is reported as PValue in tab.best, and can be used for correction across regions using the BH method to control the region-level FDR.
...
# merge.res <- mergeResults(filtered.data, res$table, tol=100, merge.args=list(max.width=4000))
# names(merge.res)
# 
# merge.res.large <- mergeResults(filtered.data.200, res.win.large$table, tol=100, merge.args=list(max.width=4000))
# names(merge.res.large)



### Weighting windows on abundance
# tab.ave <- getBestTest(merged$id, results.norep$table, by.pval=FALSE) 
...
# colnames(annot)[1:4] <- c("seqnames", "start", "end", "strand")
# annot$strand <- ifelse(annot$strand == "1","+","-")
# annot <- makeGRangesFromDataFrame(annot, keep.extra.columns = TRUE)
# # remove pseusogenes from the annotation
# annot <- annot[grep("protein_coding", annot$gene_biotype)]
...
cons <-
  mergeResultsList(
    ...
    # ranges.list = list(filtered.data, filtered.data.200),
    tab.list = list(results.norep$table, results.norep.large$table),
    tol = 1000,
    equiweight=TRUE
    # merge.args = list("PValue", "logFC")
    # merge.args=list(max.width=4000)
  )
...
# ofile <- gzfile("clusters.tsv.gz", open="w") write.table(as.data.frame(merged$region), file=ofile,
# row.names=FALSE, quote=FALSE, sep="\t") close(ofile)
...
# 
# sig_windows <- merged$regions[merged$best$PValue < 0.05]
# mcols(sig_windows) <- DataFrame(merged$best[merged$best$PValue < 0.05, ])
# 
# # load Ensembl annotation and direct annotation
# D <- distanceToNearest(sig_windows, annot, select= "all")
# # Each of the 2 combined objects has sequence levels not in the other: - Make sure to always combine/compare objects based on the same reference
# 
# sig_windows <- sig_windows[queryHits(D)]
# 
# mcols(sig_windows) <- cbind(mcols(sig_windows), mcols(annot[subjectHits(D)]), mcols(D))
# hist(as.numeric(mcols(sig_windows)$PValue))
# 
# write.table(as.data.frame(mcols(sig_windows)),
#             paste0(outPath, "Ikzf3_CnR_CSAW_fraglen195_bestPvals_mergeWin50.txt"), 
#             row.names = F)
...
```{r}
# # merge.res.large$best
# 
# sig_windows.large <- merge.res.large$regions[merge.res.large$best$PValue < 0.05]
# # merge.res$regions[merge.res$best$FDR < 0.05]
# 
# # mcols(sig_windows) <- DataFrame([cons$table$FDR < 0.05, ])
# # sig_windows <- cons$region[cons$table$FDR < 0.05]
# mcols(sig_windows.large) <- DataFrame(merge.res.large$best[merge.res.large$best$PValue < 0.05, ])
# 
# # load Ensembl annotation and direct annotation
# D <- distanceToNearest(sig_windows.large, annot, select= "all")
# # Each of the 2 combined objects has sequence levels not in the other: - Make sure to always combine/compare objects based on the same reference
# 
# sig_windows.large <- sig_windows.large[queryHits(D)]
# 
# mcols(sig_windows.large) <- cbind(mcols(sig_windows.large), mcols(annot[subjectHits(D)]), mcols(D))
# hist(as.numeric(mcols(sig_windows.large)$PValue))
# 
# write.table(as.data.frame(mcols(sig_windows.large)),
#             paste0(outPath, "Ikzf3_CnR_CSAW_fraglen195_bestPvals_mergeWin200.txt"), 
#             row.names = F)
# 
# ## 11 hits - 3 are common with MACS2 results: "Kpna2"         "Hnrnpk"        "2410131K14Rik"
# intersect(macsIkzf3$SYMBOL, mcols(sig_windows.large)$external_gene_name)
```
